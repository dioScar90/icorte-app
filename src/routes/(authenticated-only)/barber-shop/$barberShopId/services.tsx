import { FormService, RegisterProps, RemoveProps, UpdateProps } from '@/components/barber-shop-services/_form'
...
function RouteComponent() {
  const [barberShop, services] = Route.useLoaderData({
    select: (s) => [
      s.barberShop!,
      s.services!,
    ] as const
        ...
        <Card className="mx-auto max-w-sm min-w-[80vw] md:min-w-[750px] lg:min-w-[800px]">
          ...
          <CardContent className="py-4 px-2 md:px-3 lg:px-4">
            <Table>
              ...
              <TableBody>
                {services.length > 0
                  ? services.map(({ id: serviceId, barberShopId, ...service }) => (
